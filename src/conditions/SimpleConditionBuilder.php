...
class SimpleConditionBuilder extends \yii\db\conditions\SimpleConditionBuilder
{
    ...
    {
        /** @var SimpleCondition $expression */
        ...
        if ($value === null) {
            return [
                $column => [
                    $this->getOperator($operator) => null,
                ],
            ];
        }
        if ($value instanceof ExpressionInterface) {
            return [
                $column => [
                    $this->getOperator($operator) => $this->queryBuilder->buildExpression($value, $params),
                ],
            ];
        }
        ...
        return [
            $column => [
                $this->getOperator($operator) => $phName,
            ],
        ];
    }
}
